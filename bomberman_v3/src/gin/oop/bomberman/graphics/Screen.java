package gin.oop.bomberman.graphics;
...
public class Screen {
	...
	public Screen(int width, int height, Frame frame) {
		_width = width;
		_height = height;
	...
	public void drawEndGame(Graphics g, int points) {
		...
		if (!_frame._containerPanel._game._board.win) {
			drawCenteredString("YOU LOSE", getRealWidth(), getRealHeight() - 200, g);
		}
		else {
			drawCenteredString("YOU WIN", getRealWidth(), getRealHeight() - 200, g);
		}

		font = new Font("Arial", Font.PLAIN, 10 * Game.SCALE);
		g.setFont(font);
		g.setColor(Color.yellow);
		drawCenteredString("POINTS: " + points, getRealWidth(), getRealHeight() + (Game.TILES_SIZE * 2) * Game.SCALE - 200, g);
		_frame._containerPanel._playGame._gamePanel.display();
